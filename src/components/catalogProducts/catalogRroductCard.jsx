...
import rating1 from '../../assets/catalog/rating.svg'
import rating2 from '../../assets/catalog/rating2.svg'
import rating3 from '../../assets/catalog/rating3.svg'
import rating4 from '../../assets/catalog/rating4.svg'
import rating5 from '../../assets/catalog/rating5.svg'
const CatalogRroductCard = ({
  id,
  imgPrewiev,
  rating,
  title,
  ...
  if (view2 === 0) {
    return (
      <Link
        onClick={() => putId(id)}
        to="/product"
        className="col-4 products__item item product-grid"
      >
        <div className="item__content">
          <span className={`item__discount ${discount !== 0 ? 'active' : ''}`}>
            - {discount} %
          </span>
          <img
            src={imgPrewiev}
    ...
    return (
      <Link onClick={() => putId(id)} to="/product" className="product flex">
        <div className="col-4">
          <img src={imgPrewiev} alt="" className="product__img" />
        </div>
        <div className="col-4 product__col">
          ...
          <div className="product__row flex">
            <ul className="col-6 product__feautures">
              {feautures.map((item) => {
                return (
                  <li key={item} className="product__item flex">
                    {item}
                  </li>
                )
              })}
            ...
            <ul className="col-6 product__feautures">
              {property.map((item) => {
                if (item.tag) {
                  return (
                    <li key={item} className="product__item flex">
                      <span className="product__span--green">
                        {item.tagText}
                      </span>
                      {item.text}
                    </li>
        ...
        <div className="col-4 product__col">
          ...
          <span className="product__shipping product__shipping--bold">
            Free Shipping
          </span>
